...
type ResultCopy = {
  ...
  noResultBody: string;
  pasteNewNotice: string;
  trySampleNotice: string;
  backToYarn: string;
  newYarn: string;
  importantDateSingular: string;
  importantDatePlural: string;
  insightSource: string;
  model: string;
  generatedByYarnMe: string;
};
...
const copy: Record<LanguageCode, ResultCopy> = {
  "simple-english": {
    ...
    questionError: "YarnMe could not answer right now. Please try again.",
    ...
    importantDateSingular: "important date",
    importantDatePlural: "important dates",
    insightSource: "Insight Source",
    model: "Model",
    generatedByYarnMe: "Generated by YarnMe",
  },
  pidgin: {
    languageName: "Pidgin",
    ...
    questionError: "YarnMe no fit answer now. Abeg try again.",
    connectionError: "Connection no work. Abeg try again.",
    sendQuestion: "Send question",
    ...
    importantDateSingular: "important date",
    importantDatePlural: "important dates",
    insightSource: "Source insight",
    model: "Model",
    generatedByYarnMe: "YarnMe generate am",
  },
  hausa: {
    languageName: "Hausa",
    ...
    questionError: "YarnMe bai iya amsawa yanzu ba. Sake gwadawa.",
    connectionError: "Hadewa ta kasa. Sake gwadawa.",
    sendQuestion: "Aika tambaya",
    noResultTitle: "Ba a fayyace sanarwa ba tukuna",
    noResultBody:
      "Manna sanarwar hukuma ko ka zabi misali domin ganin YarnMe yana aiki.",
    pasteNewNotice: "Manna sabuwar sanarwa",
    trySampleNotice: "Gwada misali",
    backToYarn: "Koma Yarn",
    newYarn: "Fara sabon bayani",
    importantDateSingular: "rana mai muhimmanci",
    importantDatePlural: "ranaku masu muhimmanci",
    insightSource: "Tushen bayani",
    model: "Model",
    generatedByYarnMe: "YarnMe ne ya samar da shi",
  },
};
...
export function ResultScreen() {
  ...
  const currentLang = analysisResult?.language ?? settings.language;
  const activeCopy = copy[settings.language];
  const resultLanguageCopy = copy[currentLang];
  ...
  async function handleAsk(questionText: string) {
    ...
    try {
      ...
      if (answer) {
        ...
        setQaError(activeCopy.questionError);
      }
    } catch {
      setQaError(activeCopy.connectionError);
    } finally {
      setIsAsking(false);
    }
  }
    ...
    <section className="rounded-2xl border border-outline-variant/30 bg-surface-container-lowest p-md shadow-card">
      ...
      >
        ...
        <button
          ...
          aria-label={activeCopy.sendQuestion}
    ...
    >
      <div className="mx-auto w-full max-w-[1180px] px-container-margin pb-xl pt-md lg:px-lg lg:pt-lg">
        <header className="mb-lg flex items-center justify-between gap-sm">
          <button
            type="button"
            aria-label={activeCopy.backToYarn}
          ...
          <div className="relative">
            ...
            >
              ...
              <span>{languageLabels[analysisResult.language]}</span>
              ...
              <div className="flex gap-sm">
                ...
                <div>
                  ...
                  <p className="text-label-md text-on-surface">
                    ...
                    {analysis.dates.length === 1
                      ? activeCopy.importantDateSingular
                      : activeCopy.importantDatePlural}
                  </p>
                </div>
              </div>
              <div className="flex gap-sm">
